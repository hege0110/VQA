...
from features import get_questions_matrix_sum, get_images_matrix, get_answers_matrix,get_images_matrix_from_VGGmodel
...
def main():
	...
	parser.add_argument('-answers_train',type=str, default = cwd+'/data/preprocessed/answers_train2014_modal.txt')
	#parser.add_argument('-questions_train',type=str, default = cwd+'/data/preprocessed/questions_train2014.txt')
	...
	if args.language_only:
		model_file_name = cwd+'/models/mlp_language_only_num_hidden_units_' + str(args.num_hidden_units) + '_num_hidden_layers_' + str(args.num_hidden_layers)
	else:
	...
	for k in xrange(args.num_epochs):
		...
		for qu_batch,an_batch,im_batch in zip(grouper(questions_train, args.batch_size, fillvalue=questions_train[-1]), 
			...
			if args.language_only:
				...
				X_i_batch = get_images_matrix_from_VGGmodel(im_batch,id_map)
